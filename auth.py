...
def is_owner_session():
    """Return True only for an authenticated session explicitly marked owner.

    Authorization deliberately fails closed. In particular, an old signed
    session created before roles existed must never acquire owner authority
    merely because its role field is absent.
    """
...
def owner_required(f):
    ...
    Missing or unknown roles are denied rather than being interpreted as owner.
    """
    @wraps(f)
    def decorated(*args, **kwargs):
        if not session.get('logged_in'):
