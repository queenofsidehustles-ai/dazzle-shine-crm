...
with app.app_context():
    ...
    # boundary on PostgreSQL, keeping the control plane distinct from a
    # tenant's own schema. SQLite has no concept of multiple schemas in one
    # database, so a schema-qualified query fails outright rather than
    # finding nothing; this translates it away for this connection only, and
    # creates the (now schema-free) table so a lookup for a company that
    # does not exist behaves like PostgreSQL does when it doesn't -- returns
