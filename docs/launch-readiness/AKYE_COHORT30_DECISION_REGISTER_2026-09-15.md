...
| DR-016 | Treat Launch Readiness run `35017255542` on exact head `7d58fa0f36768eb9a84e0e6dafa8105505f03419` as authoritative GREEN evidence for the existing critical backend gates. The prior red run was workflow diagnostic-control drift, not a runtime regression. | The corrected workflow preserved every substantive gate and the fail-closed aggregator; the exact-head rerun passed PAY-01/PAY-02, tenant/media/lifecycle, PostgreSQL/30-tenant/backup, scheduler/cron/provider, CSRF, RBAC/session, provisioning and forwarded-host gates. Avoids unnecessary runtime changes while retaining fail-closed release control. | Reopen only on a later exact-head regression or new falsification evidence. |
...
- Privacy lifecycle falsification is GREEN on exact head `7d58fa0f36768eb9a84e0e6dafa8105505f03419` (run `35017255544`).
- Launch Readiness is GREEN on exact head `7d58fa0f36768eb9a84e0e6dafa8105505f03419` (run `35017255542`).
...
- The prior exact-head red run on `a330965afb830af665788066ef596de7f3ffa0d4` was caused by diagnostic predicates that executed after successful `continue-on-error` gates. Commit `7d58fa0f36768eb9a84e0e6dafa8105505f03419` corrected those predicates without weakening substantive tests or the final aggregator.
- The earlier PAY-02 and tenant HTTP/media grouped blocker is closed by exact-head evidence; no runtime-code relaxation is warranted.
...
- A real scheduled Akye backup run (`34972689108`, run #27) completed successfully with scratch PostgreSQL restore proof, encryption, uploaded backup artifact and uploaded manifest; this materially closes the operational backup-execution gap while release rehearsal/RTO evidence remains separately reviewable.
...
- `tests/mobile-accessibility-golden.spec.js` adds executable local-only viewport/accessibility falsification for critical owner journeys. It has not yet been claimed PASS: deterministic CI execution evidence is still required.
...
1. Make the mobile/accessibility Golden Journey deterministic in CI, execute it, and correct only launch-blocking defects demonstrated by it; extend to cleaner-role critical journeys where the existing local harness can do so safely.
2. Consolidate operational backup/rollback evidence, including explicit RTO/release rehearsal evidence, without conflating it with code-level containment.
