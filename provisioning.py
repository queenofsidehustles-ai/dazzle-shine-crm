...
def main():
    ...
    d = sub.add_parser('destroy', help='close a company now; permanent purge is retention-gated')
    ...
    if args.action == 'create':
        ...
        if args.csv:
            ...
            return 0
        print(f'\n  {len(rows)} early-access request' f'{"s" if len(rows) != 1 else ""}, newest first\n')
        for r in rows:
            when = r['created_at'].strftime('%d %b, %H:%M') if r['created_at'] else ''
            print(f'  {r["name"] or "(no name)"}' f'{" — " + r["company"] if r["company"] else ""}   [{when}]')
            print(f'     {r["email"] or ""}' f'{"   " + r["phone"] if r["phone"] else ""}' f'{"   " + r["cleaners"] + " cleaners" if r["cleaners"] else ""}')
        ...
        if ok:
            print(f'\n  ✅ Accepted by the provider: {detail}\n'); return 0
        print(f'\n  ❌ Not sent: {detail}\n'); return 1
    elif args.action == 'nudges':
        import trial_nudges
        try: control_plane.ensure_table(engine)
        except Exception:
            print('\n  This deployment has no control plane — nothing to nudge.\n'); return 0
        counts = trial_nudges.run(engine, dry_run=args.dry_run)
        plan = counts.get('plan') or []; head = 'Would send' if args.dry_run else 'Sent'
        print(f'\n  {counts["considered"]} companies checked.')
        if not plan:
        ...
        print(f'\n  This closes {org["name"]} immediately. Data is retained for 30 days.')
        print('  Permanent deletion is not available through this command.\n')
        ...
        print(f'  {org["name"]} closed. Earliest permanent purge: {state["eligible_at"].isoformat()}.\n')
        return 0
    return 0
