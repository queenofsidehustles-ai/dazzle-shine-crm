...
TIP_FEE_PCT_DEFAULT  = 2.9  # card processing taken off a tip before it's handed on
...
def get_tip_fee_percent():
    """Card processing deducted from a tip before it reaches the cleaner.
    Stripe's percentage only — the flat per-transaction cent isn't included,
    because that would have been charged on the cleaning anyway."""
    return _db_get('tip_fee_pct', TIP_FEE_PCT_DEFAULT)
